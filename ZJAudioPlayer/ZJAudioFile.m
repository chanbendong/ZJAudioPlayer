...
- (BOOL)_openAudioFile
{
    OSStatus status = AudioFileOpenWithCallbacks((__bridge void *)self, ZJAudioFileReadCallBack, NULL, ZJAudioFileGetSizeCallBack, NULL, _fileType, &_audioFileID);
    if (status != noErr) {
        _audioFileID = NULL;
        return NO;
    }
...
- (void)_calculatePacketsDuration
{
...
- (NSArray *)parseData:(BOOL *)isErr
{
    ...
    if (ioNumPackets > 0) {
        ...
        for (int i = 0; i < ioNumPackets; i++) {
            ...
            ZJParseAudioData *parsedData = [ZJParseAudioData parsedAudioDataWithBytes:outBuffer+packetDescription.mStartOffset packetDescription:packetDescription];
...
static SInt64 ZJAudioFileGetSizeCallBack(void *inClientData){
    ZJAudioFile *audioFile = (__bridge ZJAudioFile *)inClientData;
